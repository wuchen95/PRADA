...
from marcodoc.dataset import MSMARCOSURRMODELDataset, get_collate_function, MSMARCODataset
...
def train(args, model):
    ...
    train_dataset = MSMARCOSURRMODELDataset("train", args.msmarco_dir,
        ...
        for step, (batch, _, _) in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.evaluate_during_training and (
                    ...
                    mrr = evaluate(args, model, mode="dev",
...
def evaluate(args, model, mode, prefix):
    ...
    eval_dataloader = DataLoader(eval_dataset, batch_size=args.eval_batch_size,
                                 num_workers=args.data_evaluate_num_workers,
                                 collate_fn=collate_fn)
...
def run_parse_args():
    ...
    args.log_dir = f"{args.output_dir}/surr_model_log/{time_stamp}"
    args.model_save_dir = f"{args.output_dir}/surr_models"
    args.eval_save_dir = f"{args.output_dir}/surr_eval_results"
    return args
...
def main():
    ...
    if args.task == "train":
        ...
        result = evaluate(args, model, args.task,
                          prefix=f"ckpt-{args.eval_ckpt}")
        print(result)
